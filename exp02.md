...
1. 从统一定义的接口类型文件(Types.ts)中引入(import)需要的接口，约束对象数据结构
2. 先声明是 变量let/常量const (多数时候用const)
3. 再声明 变量名称 : 变量类型
4. const courses: Course[]
5. 先声明是常量(const)，
6. 再声明变量名称(courses)，
7. 最后声明变量类型(: Course[]) （Course类型的数组）
...
1. ref 是 Vue 3 提供的响应式引用函数，用于**创建响应式数据**
2. ref<T>()支持将基本数据类型/对象/数组转为响应式数据
3. 与 watch、computed 等配合实现响应式逻辑
4. 以便在数据变化时触发视图更新等响应式行为
...
1. <类型>为 TypeScript 的类型标注（可选，推荐使用以增强代码严谨性）
2. 初始值为创建ref时的默认值
...
ref 返回的是一个包含 value 属性的响应式对象,在 JavaScript/TypeScript 代码中,需要通过` .value` 来访问和修改其封装的响应式数据
...
当对象属性为可选属性时，属性可能为空,可使用`可选链操作符 ?.` 避免空指针异常
...
v-model="selectedCoursesR"将复选框与selectedCoursesR响应式数组进行双向绑定：

当勾选复选框时，对应的课程对象会被添加到selectedCoursesR数组中；当取消勾选时，对应的课程对象会从selectedCoursesR数组中移除。
...
1. v-for="(item, index) in items" :key="item.id"
2. item为数组中的每个元素，index为元素的索引（可选）
3. items为被遍历的数组，key为每个元素的唯一标识
...
color样式的值会根据totalScoreR与maxScore的比较结果动态变化：

当总学分大于等于最大学分时，文字颜色为绿色；否则为红色
